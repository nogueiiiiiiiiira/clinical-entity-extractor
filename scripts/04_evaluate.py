...
from utils import (
    padronizar_string, normalizar_termo_texto, normalizar_para_match, expansion_of,
    fuzzy_partial_match, llm_semantic_match, load_json_cache, save_json_cache
)

def extrair_gold_terms(root, narrativa_filename):
    """Extrai termos do gold standard a partir do XML anotado."""
    achados = []
    ...
    if relations_elem is not None:
        for rel in relations_elem:
            if rel.tag == 'RELATION':
                ...
                if an1 and an2:
                    rel_dict.setdefault(an1, []).append({'id_relacionado': an2, 'tipo_relacionamento': tipo})
            ...
            for value in value_list:
                if id_anot == value['id_relacionado']:
                    ...
                    if anot_rel is not None:
                        if anot_rel.get('Polaridade') == 'Negativa' or value['tipo_relacionamento'] == 'negation_of':
                            negado = True
...
def classificar_erro(pred_term, gold_term, texto_narrativa, predicted_all, gold_all, tipo_erro, categoria_pred=None, categoria_gold=None):
    """Classifica o tipo de erro para FP ou FN."""
    ...
    verbos_extras = [
        'apresenta', 'refere', 'relata', 'queixa', 'evoluiu', 'nega', 'sem', 'ausência',
        'relatado', 'descreve', 'informa', 'nota', 'observa', 'constata'
    ]
    if termo_norm in verbos_extras or (len(termo_norm) < 4 and termo_norm not in ['has','icc','dm','da','dc','rv']):
        return "Extração de verbo ou ruído"
    if tipo_erro == 'FP':
        for gt in gold_all:
...
def extrair_contexto(texto, termo, window=80):
    """Extrai trecho do texto ao redor do termo para contexto."""
    if not termo:
        return ""
...
def avaliar_modo(csv_path, modo, output_suffix):
    """Executa a avaliação em um modo específico (strict ou relaxed)."""
    print(f"Iniciando avaliação no modo {modo.upper()} (sufixo: {output_suffix})")
    df_prompts = pd.read_csv(csv_path)
    if 'polaridade' in df_prompts.columns:
        df_prompts = df_prompts[df_prompts['polaridade'].str.strip().str.lower() == 'positiva']
    narrativas_unicas = df_prompts['nomeNarrativa'].unique()
    df_resultado = pd.DataFrame(columns=[
        "nomeNarrativa", "textoPrompt", "categoria", "termoAnalisado", "abreviacao",
        "SCTID", "CID10", "expansao_correta", "SCTID_correto", "CID10_correto",
        "semClin_nomeNarrativa", "semClin_textoAnalisado", "semClin_categoria", "classificacao"
    ])
    ...
    for narrativa_atual in narrativas_unicas:
        ...
        if df_narr.empty:
            continue
        achados_prompt = df_narr[['termoAnalisado', 'textoPrompt', 'categoria', 'abreviacao',
                                  'SCTID', 'CID10', 'expansao_correta', 'SCTID_correto', 'CID10_correto']].to_dict('records')
        nome_base = narrativa_atual.replace('.xml', '') if narrativa_atual.endswith('.xml') else narrativa_atual[:4]
        caminho_gold = os.path.join(Config.GOLDSTANDARD_FOLDER, f"{nome_base}_goldstandard.xml")
        if not os.path.exists(caminho_gold):
            print(f"Gold standard não encontrado para {narrativa_atual}. Pulando.")
            continue
        ...
        try:
            ...
            print(f"Erro ao ler gold standard {caminho_gold}: {e}")
            continue
        for p in achados_prompt:
            p['termo_norm'] = normalizar_para_match(p['termoAnalisado'])
        ...
        for i, pred in enumerate(achados_prompt):
            for j, gold in enumerate(achados_semclin):
                ...
                if pred['termo_norm'] == gold['termo_norm']:
                    ...
                    if (expansion_of(pred['termo_norm'], gold['termo_norm']) or
                        expansion_of(gold['termo_norm'], pred['termo_norm'])):
        ...
        for _, i, j, _ in matches:
            ...
            nova = {
                ...
                "termoAnalisado": pred['termoAnalisado'],
                "abreviacao": pred['abreviacao'],
                "SCTID": pred.get('SCTID', ''),
                "CID10": pred.get('CID10', ''),
                "expansao_correta": pred.get('expansao_correta', ''),
                "SCTID_correto": pred.get('SCTID_correto', ''),
                "CID10_correto": pred.get('CID10_correto', ''),
    ...
    print(f"Planilha detalhada salva em {excel_path}")
    ...
    pd.DataFrame([metricas]).to_csv(os.path.join(eval_dir, f"tabela2_contagem_geral_{output_suffix}.csv"), index=False)
    pd.DataFrame([{"Modelo": f"Extração - modo {modo}", "Precisão": prec, "Recall": rec, "F1": f1}]).to_csv(
        os.path.join(eval_dir, f"tabela5_comparacao_geral_{output_suffix}.csv"), index=False)
    linhas_tabela3 = []
    linhas_tabela6 = []
    ...
    pd.DataFrame(linhas_tabela3).to_csv(os.path.join(eval_dir, f"tabela3_contagem_por_categoria_{output_suffix}.csv"), index=False)
    pd.DataFrame(linhas_tabela6).to_csv(os.path.join(eval_dir, f"tabela6_detalhamento_categoria_{output_suffix}.csv"), index=False)
    erros_por_tipo = {}
    for _, row in df_resultado.iterrows():
        ...
        todos_pred = df_resultado[df_resultado['nomeNarrativa'] == narrativa]['termoAnalisado'].dropna().tolist()
        todos_gold = df_resultado[df_resultado['nomeNarrativa'] == narrativa]['semClin_textoAnalisado'].dropna().tolist()
        tipo = classificar_erro(pred_term, gold_term, texto, todos_pred, todos_gold, classificacao,
                                categoria_pred, categoria_gold)
        erros_por_tipo.setdefault(tipo, []).append(row)
    resumo_erros = []
    for tipo, rows in erros_por_tipo.items():
        amostra = rows[0]
        termo_exibido = amostra['termoAnalisado'] if amostra['classificacao'] == 'FP' else amostra['semClin_textoAnalisado']
        contexto = extrair_contexto(amostra['textoPrompt'], termo_exibido) if amostra['textoPrompt'] else ""
        resumo_erros.append({
            "Tipo de erro": tipo,
    ...
    pd.DataFrame(resumo_erros).to_csv(os.path.join(eval_dir, f"erros_classificados_{output_suffix}.csv"), index=False)
    print(f"Avaliação no modo {modo} concluída.")

def main():
    """Executa a avaliação nos modos strict e relaxed."""
    consolidated_csv = os.path.join(Config.OUTPUT_BASE, "consolidated_terms.csv")
    if not os.path.exists(consolidated_csv):
        print("Arquivo consolidado não encontrado. Execute 03_merge_results.py primeiro.")
        return
    ...
    print("Avaliação completa (modos estrito e relaxado) finalizada.")
