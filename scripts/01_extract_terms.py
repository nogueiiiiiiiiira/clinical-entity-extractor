...
from utils import (
    ...
    load_json_cache,
    save_json_cache,
    verificar_expansao_hibrida,
    resolver_conflito_expansao,
    salvar_annotations_json,
    Tee,
    _save_llm_response,
    load_abreviacoes,
    post_process_entities,
    consolidar_annotations_semantic
)
...
def consolidar_annotations(lista_de_listas: list, narrative_name: str, texto_original: str) -> list:
    ...
    for ann_list in lista_de_listas:
        processed = post_process_entities(ann_list, ABREVIACOES_CACHE)
        all_annotations.extend(processed)

    consolidado = consolidar_annotations_semantic(all_annotations, threshold=0.85)

    melhores = {}
    for ann in consolidado:
